...
stats.sort_stats(pstats.SortKey.TIME) # Tri par temps CPU total

print("\n=== 🔍 Fonctions les plus coûteuses ===")
stats.print_stats(20)

# Sauvegarde des résultats détaillés
stats.dump_stats(PROFILE_PATH) # Sauvegarde dans un fichier .prof
...
# Charger le modèle (comme dans app.py)
model = joblib.load("xgboost_features90.joblib")

# Récupération des colonnes attendues si le modèle les contient
expected_cols = None
if hasattr(model, "get_booster"):
    expected_cols = model.get_booster().feature_names

# --- Fonction interne à profiler (extrait de app.py, sans Gradio) ---
def predict_from_df(df: pd.DataFrame):
    ...
    return df[["prediction_label"]]

# --- Génération d’un échantillon simulé (remplace le CSV) ---
# Tu peux aussi lire ton vrai CSV ici
sample_data = {
    "EXT_SOURCE_2": [0.55, 0.22, 0.68],
    "EXT_SOURCE_3": [0.38, 0.12, 0.44],
    "NAME_EDUCATION_TYPE": [3, 2, 4],
    "CODE_GENDER_0_0": [0, 1, 0],
    "FLAG_DOCUMENT_3": [1, 1, 0],
}
df = pd.DataFrame(sample_data)

# --- Fonction de test du profiling ---
def test_prediction():
    """Exécute plusieurs prédictions pour stabiliser la moyenne"""
    for _ in range(20):
        predict_from_df(df.copy())
...
print("\n=== 20 FONCTIONS LES PLUS COÛTEUSES ===")
stats.print_stats(20)

# Sauvegarde des résultats complets pour analyse dans SnakeViz ou pstats
stats.dump_stats("cProfile/profiling_results.prof")

print("\nProfiling terminé. Résultats enregistrés dans cProfile/profiling_results.prof")
'''
